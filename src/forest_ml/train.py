...
def get_params()->dict:
    ...
    parser.add_argument('--path-save-model', type=pathlib.Path, required=False, default='models/model.joblib',
                        help='A path where to save the trained model')
    parser.add_argument('--random-state', type=int, required=False, default=42,
                        help='random_state for train test split, model training and etc., must be an integer')
    parser.add_argument('--test-split-ratio', type=int, choices=range(0, 1), required=False, default=0.3,
                        help='Test data ratio, 0.3 by default')
    parser.add_argument('--use-scaler', type=bool, required=False, default=False,
                        help='Whether to use a scaler on data or not, False by default')
    parser.add_argument('--model', type=str, required=False,
                        choices=['RandomForestClassifier', 'LogisticRegression', 'KNeighborsClassifier',
                                 'ExtraTreesClassifier'], default='RandomForestClassifier',
                        help='What machine learning model to use')
    parser.add_argument('--fetengtech', type=str, required=False, choices=['1', '2'], default='1',
                        help='What feature engineering technique to use')

    # hyperparameters for random forest classifier
    parser.add_argument('--max-depth', type=int, required=False, default=None,
                        help='hyperparameter for random forest and extratreeclassifier')
    parser.add_argument('--min-sample-split', type=int, required=False, default=None,
                        help='hyperparameter for random forest and extratreeclassifier')
    parser.add_argument('--max-leaf-nodes', type=int, required=False, default=None,
                        help='hyperparameter for random forest and extratreeclassifier')
    parser.add_argument('--min-samples-leaf', type=int, required=False, default=None,
                        help='hyperparameter for random forest and extratreeclassifier')
    parser.add_argument('--n-estimators', type=int, required=False, default=None,
                        help='hyperparameter for random forest and extratreeclassifier')
...
def train() -> None:
    ...
    X, y = get_dataset(
        csv_path=params['path_to_dataset'], split_into_train_test=False, random_state=params['random_state'],
